...
class SportHealthBot:
    ...
    async def show_main_menu(self, update, ctx):
        ...
        # Тренер-блок (виден тренеру — ограниченная роль, только свои команды)
        elif self.db.is_coach(user.id):
            buttons.insert(0, [(f"👕 Моя команда", "coach_team_view")])
            buttons.insert(1, [(f"📊 Мой отчёт (Excel)", "report_export_menu")])
    ...
    async def scale_import_menu(self, update, ctx):
        ...
        await q.edit_message_text(
            ...
            "1️⃣ Взвесь группу на весах (16 профилей).\n"
    ...
    async def bc_show_profiles(self, update, ctx):
        """Показать/закрепить сопоставление профилей весов → спортсмены."""
        q = update.callback_query
        await q.answer()
        if not self._is_admin_or_coach(q.from_user.id):
            return
        ...
        # профили с 1 по 16
        btns = [[(f"Профиль {n}", f"bc_map_{n}") for n in range(i, min(i + 4, 17))]
    ...
    def _scoped_athletes(self, user_id, athletes=None):
        ...
        Админ/врач → все. Тренер → только спортсмены его команд (пусто, если нет команд).
        Обычный пользователь → пусто."""
        if self._is_full_access(user_id):
            return athletes if athletes is not None else self.db.get_all_athletes()
        teams = set(self.db.get_coach_teams(user_id))
        if not teams:
            return []
    ...
    async def _finish_survey(self, source, user_id, state):
        ...
        try:
            for admin_id in self._full_access_ids():
                if admin_id != user_id:  # Не дублируем тому кто прошел
                    ...
                    # Кнопка ответа спортсмену — если есть жалобы или отклонения
                    need_reply = bool(complaints) or (recs and recs != "✅ Все показатели в норме!")
                    admin_buttons = ([[(f"📩 Ответить {athlete_name}", f"reply_athlete_{athlete['id']}")]]
        ...
        try:
            ...
            if _flags:
                ...
                _alert_btn = [[(f"📩 Ответить {athlete.get('full_name', '?')}", f"reply_athlete_{athlete['id']}")]]
                ...
                if qd:
                    ...
                    if surgery and surgery not in ("Нет", "нет", "-"):
                        ...
                        if pain_nrs is not None and pain_nrs >= 3:
                            recs.append(f"🏥 *Операция в анамнезе ({surgery[:60]}) + боль сегодня ({pain_nrs}/10)*\n• Обязательно показать врачу, даже если боль слабая")
    ...
    async def set_reminder_time(self, update, ctx):
        ...
        if self.job_queue:
            ...
            # Убираем старую рассылку с тем же именем, чтобы не было дублей при перепланировке
    ...
    async def admin_complaints_page(self, update, ctx):
        ...
        for c in complaints[:15]:  # лимит чтобы не переполнить
            ...
            btns.append([(f"💬 Ответить: {c['full_name']}", f"reply_athlete_{c['id']}")])
    ...
    async def report_export_menu(self, update, ctx):
        ...
        if is_admin:
            teams = set(a["team"] for a in self.db.get_all_athletes() if a.get("team"))
        else:
            teams = self._coach_teams(uid)
    ...
    async def generate_xlsx_report(self, update, ctx):
        ...
        for t in all_teams:
            ...
            metrics = ["Сон", "Утомл", "Боль", "Пульс", "sRPE", "Hooper"]
    ...
    async def _send_daily_reminder(self, context):
        ...
        # Дедупликация по дню: если напоминание уже отправлялось сегодня,
        # не слать повторно (в т.ч. после рестарта бота). Считаем дату по tz напоминания.
        try:
            import pytz
            from datetime import datetime as _dthr
            _tz = pytz.timezone(REMINDER_TZ)
            _today = _dthr.now(_tz).date().isoformat()
        except Exception:
            _today = date.today().isoformat()
        if self.db.get_setting("reminder_sent_date") == _today:
            logger.info("Reminder already sent today — skip")
            return
        ...
        # Сводка врачу — используем тот же batch
        try:
            passed = [a for a in athletes if survey_map.get(a["id"], False)]
            not_passed = [a for a in athletes if not survey_map.get(a["id"], False)]
            summary = (
    ...
    def run(self):
        ...
        if REMINDER_HOUR is not None:
            ...
            # Если время уже прошло сегодня — отправляем сразу (но не чаще раза в день)
            if now_local.hour >= REMINDER_HOUR and now_local.minute >= REMINDER_MINUTE:
                # если сегодня уже отправлено — не дублировать при рестарте
                already = self.db.get_setting("reminder_sent_date") == now_local.date().isoformat()
                if already:
                    logger.info("Время напоминания прошло, но сегодня уже отправлено — не дублирую")
                else:
